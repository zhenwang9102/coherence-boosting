...
class CommitmentBank(MultipleChoiceTask):
    def __init__(self, add_description, num_shots, rnd, **kwargs):
        super().__init__()
        self.task_name = "CommitmentBank"
    ...
    def load_data(self):
        self.data = datasets.load_dataset('super_glue', 'cb')
    ...
    def standardize(self, sample):
        ...
        cur_data['context'] = " Given question: {} Is {} true, false, or neither?\n The answer is:".format(sample['premise'], sample['hypothesis'])
    ...
    def get_contrast_ctx(self, batch):
        prompts = []
        for data in batch:
            choices = [" Is {} true, false, or neither?\n The answer is:".format(data['hypothesis'])] * len(data['choices'])
            prompts += choices
